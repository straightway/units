...
class UnitValueTest {
    ...
    @Test
    fun construction_value() = assertEquals(2, UnitValue(2, testUnit).value)

    @Test
    fun construction_unit() = assertEquals(testUnit, UnitValue(2, testUnit).unit)

    @Test
    fun construction_withIndexer_value() = assertEquals(2, 2[testUnit].value)

    @Test
    fun construction_withIndexer_unit() = assertEquals(testUnit, 2[testUnit].unit)

    @Test
    fun construction_withScaledIndexer_value() = assertEquals(2, 2[kilo(testUnit)].value)

    @Test
    fun construction_withScaledIndexer_unit() = assertEquals(kilo(testUnit), 2[kilo(testUnit)].unit)

    @Test
    fun baseValue() = assertEquals(2000, 2[kilo(testUnit)].baseValue)

    @Test
    fun baseValue_shifted() = assertEquals(3, 2[TestQuantity(uni, 1)].baseValue)

    @Test
    fun baseValue_scaled_shifted() = assertEquals(2001, 2[kilo(TestQuantity(uni, 1))].baseValue)

    @Test
    fun toString_unscaled() = assertEquals("2 TU", 2[testUnit].toString())

    @Test
    fun toString_scaled() = assertEquals("2 kTU", 2[kilo(testUnit)].toString())

    @Test
    fun equals_sameQuantity_sameScale() = assertTrue(2[testUnit] == 2[testUnit])

    @Test
    fun equals_sameQuantity_differentScale() = assertTrue(2000[testUnit] == 2[kilo(testUnit)])
    ...
    @Test
    fun equals_differentTypes() = assertFalse(2000[testUnit].equals("Hallo"))

    @Test
    fun equals_differentProducts() = assertFalse(2000[meter * second] == 2000[mol * ampere])

    @Test
    fun equals_shiftedUnit() = assertTrue(0[TestQuantity(uni, 1)] == 1[TestQuantity(uni, 0)])

    @Test
    fun compare_sameScale_true() = assertTrue(1[testUnit] < 2[testUnit])

    @Test
    fun compare_sameScale_false() = assertFalse(2[testUnit] < 1[testUnit])

    @Test
    fun compare_differentScale() = assertTrue(2[testUnit] < 1[kilo(testUnit)])

    @Test
    fun compare_differentNumberTypes() = assertTrue(1.0[testUnit] < 2[testUnit])

    @Test
    fun siCorrectedUnit_value() = assertEquals(2, 2[siCorrectedTestUnit].value)

    @Test
    fun siCorrectedUnit_scaledValue() = assertEquals(2.0, 2.0[siCorrectedTestUnit].baseValue)

    @Test
    fun siCorrectedUnit_baseValue() = assertEquals(2e3, 2.0[siCorrectedTestUnit withScale uni].baseValue)
