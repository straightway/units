...
fun main(args: Array<String>) {
    ...
    println("acceleration for $speed and ${14[second]} is ${accelerationFromSpeedAndTime(speed, 14[second])}")
    ...
    // to adapt the scale.
    println("${1[kilo(gram)]} is equal to ${1[kilo(gram)][pound]}") // Out: 1 kg is equal to 2.2046226218487757 lbs
    ...
    try {
        ...
        println("3[second][kelvin] throws $ex") // Out: 3[second][kelvin] throws Panic: Incompatible units: K for 3 s
    }

    // It is possible to access the value and the base value of a UnitValue:
    println("The value of ${2[kilo(meter)]} is ${2[kilo(meter)].value}") // Out: The value of 2 km is 2
    println("The base value of ${2[kilo(meter)]} is ${2[kilo(meter)].baseValue}") // Out: The base value of 2 km is 2000

    // The special cases of the unit system are handled properly, especially for mass, for which kg is the SI base
    // unit and not g, and for time, for which the base unit is second!
    println("The base value of ${2[gram]} is ${2[gram].baseValue}") // Out: The base value of 2 g is 0.002
    println("The base value of ${2[minute]} is ${2[minute].baseValue}") // Out: The base value of 2 min is 120

    // It is possible to constrain the number type for a unit value. If you e.g. specify Int as number type,
    // all calculations are done with integer. Example:
...
private fun accelerationFromSpeedAndTime(speed: UnitNumber<Speed>, time: UnitNumber<Time>): UnitNumber<Acceleration> {
    // Due to restrictions of the kotlin language, the product of units is unfortunately not commutative, as it is in
    // reality. But it is possible to convert 'compatible' units, e.g. This statement line would throw an exception
    // at runtime if the conversion is not possible. In the case shown here, the unit is properly converted. The
    // conversion is necessary, because the predefined unit type 'Acceleration' is
    ...
    // Since these two types are formally different, a conversion is required to match the function's return type.
